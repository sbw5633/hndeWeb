...
class DashboardDataHandler {
  ...
  void _handleNoticesUpdate(QuerySnapshot snapshot, AppUser user) {
    ...
    final filteredNotices = snapshot.docs
        ...
        .where((post) {
          ...
          final hasPermission = requiredLevel >= user.permissionLevel.level;
  ...
  bool _isTargetGroupMatch(String targetGroup, String userAffiliation, AppUser user) {
    ...
    // 지점 관리자 이상은 본인 소속과 전체 공지사항 확인 가능
    if (user.permissionLevel >= PermissionLevel.branchAdmin) {
      return targetGroup == '전체' || targetGroup == userAffiliation;
    }
    
    // 일반 직원은 본인 소속 공지사항만 확인 가능
    return targetGroup == userAffiliation;
  }
